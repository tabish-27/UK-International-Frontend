...
function Hero() {
  ...
  const [formData, setFormData] = useState({ name: "", phone: "", city: "", course: "" });
  const [errors, setErrors] = useState({});
  const [isPopupOpen, setIsPopupOpen] = useState(false);
  const [popupSource, setPopupSource] = useState("");

  const validateName = (name) => !name.trim() ? "Name is required" : /\d/.test(name) ? "Name cannot contain numbers" : "";
  const validatePhone = (phone) => {
    if (!phone.trim()) return "Phone number is required";
    return /^\+?\d{10}$/.test(phone.replace(/\s+/g, "")) ? "" : "Enter a valid phone number (+91 followed by 10 digits)";
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.city.trim() && !formData.course.trim()) newErrors.cityOrCourse = "Please fill either City or Course";
  ...
  const handleInputChange = (e) => {
    ...
    setFormData((prev) => ({ ...prev, [name]: name === "name" ? value.replace(/\d/g, "") : value }));
    if (errors[name]) setErrors((prev) => ({ ...prev, [name]: "", general: "", cityOrCourse: "" }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!phone.startsWith("+91")) {
      phone = `+91${phone.replace(/^\+?/, "").slice(0, 10)}`; // Ensure exactly 10 digits after +91
    }

    try {
      // Updated URL for production backend
      const response = await fetch(`https://ukinternationalbeautyschool.com/api/user/register`, {
        method: "POST",
      ...
      //       method: "POST",
      //       headers: { "Content-Type": "application/json" },
      //       body: JSON.stringify({ name: formData.name, phone }),
      //     });
  ...
  const openPopup = (source) => { setPopupSource(source); setIsPopupOpen(true); };
  const closePopup = () => { setIsPopupOpen(false); setPopupSource(""); };

  useEffect(() => {
    document.querySelectorAll("form").forEach((f) => f.setAttribute("autocomplete", "off"));
    document.querySelectorAll("input").forEach((i) => { i.setAttribute("autocomplete", "new-password"); i.setAttribute("data-form-type", "other"); });
  }, []);
          ...
          <div className="backdrop-blur-0 bg-gray-900/40 border border-white/20 rounded-2xl p-2 md:p-8 shadow-2xl">
            ...
            {(errors.general || errors.cityOrCourse) && (
              <div className="mb-4 p-3 bg-red-500/20 border border-red-500/50 rounded-lg">
                <p className="text-red-200 text-sm">{errors.general || errors.cityOrCourse}</p>
              </div>
            )}

            <form onSubmit={handleSubmit} className="space-y-3" autoComplete="new-password" data-form-type="other">
              <div>
                <label htmlFor="name" className="block text-sm font-medium text-gray-200 mb-1">Full Name *</label>
                <input type="text" id="name" name="name" value={formData.name} onChange={handleInputChange} placeholder="Enter your full name" className={`w-full px-4 py-3 bg-white/10 border rounded-lg text-white placeholder-gray-300 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-transparent transition-all duration-200 ${errors.name ? "border-red-500/50" : "border-white/20"}`} />
                {errors.name && <p className="text-red-400 text-sm mt-1">{errors.name}</p>}
              </div>

              <div>
                <label htmlFor="phone" className="block text-sm font-medium text-gray-200 mb-1">Phone Number *</label>
                <input type="tel" id="phone" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="Enter your phone number" className={`w-full px-4 py-3 bg-white/10 border rounded-lg text-white placeholder-gray-300 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-transparent transition-all duration-200 ${errors.phone ? "border-red-500/50" : "border-white/20"}`} />
                {errors.phone && <p className="text-red-400 text-sm mt-1">{errors.phone}</p>}
              </div>

              <div>
                <label htmlFor="city" className="block text-sm font-medium text-gray-200 mb-1">City</label>
                <input type="text" id="city" name="city" value={formData.city} onChange={handleInputChange} placeholder="Enter your city" className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-300 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-transparent transition-all duration-200" />
              </div>

              <div>
                <label htmlFor="course" className="block text-sm font-medium text-gray-200 mb-1">Course</label>
                <input type="text" id="course" name="course" value={formData.course} onChange={handleInputChange} placeholder="e.g., Cosmetology, Makeup" className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-300 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-transparent transition-all duration-200" />
              </div>
